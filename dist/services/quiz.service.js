...
async function recomputeUserPoints(tx, userId) {
    ...
    const totalPoints = grouped.reduce((sum, g) => sum + (g._max.points ?? 0), 0);
...
async function checkAndUnlockBadgesTx(tx, userId) {
...
async function evaluateQuiz(userId, chapterId, answers, quizType = "SOLO") {
    ...
    for (const a of answers) {
        ...
        let pickedIds = [];
        if (a.answerId)
            pickedIds = [a.answerId];
        if (a.selectedAnswerId)
            pickedIds = [a.selectedAnswerId];
        if (Array.isArray(a.answerIds))
            pickedIds = a.answerIds;
        if (Array.isArray(a.selectedAnswerIds))
            pickedIds = a.selectedAnswerIds;
